"""BigCase 신규 카테고리 수집 — Next.js data API 직접 활용
...
Usage:
    python scripts/bigcase_new_categories_collect.py
    python scripts/bigcase_new_categories_collect.py --mode full
    python scripts/bigcase_new_categories_collect.py --category sexual_harassment
    python scripts/bigcase_new_categories_collect.py --target 500
"""
import sys
import os
import json
...
def collect_category(build_id, cat_key, config, existing_keys, target=500, mode="full"):
    """카테고리 수집"""
    print(f"\n{'='*60}")
...
def main():
    parser = argparse.ArgumentParser(description="BigCase 신규 카테고리 수집")
    ...
    parser.add_argument("--target", type=int, default=500, help="카테고리당 목표 건수")
