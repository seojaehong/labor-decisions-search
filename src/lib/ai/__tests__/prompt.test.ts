...
describe('trimHistory', () => {
  ...
  it('메시지 10개에서는 slice 이후 마지막 user만 교체하고 assistant 역할은 유지한다', () => {
    const messages = [
    ...
    expect(trimmed.at(-1)?.role).toBe('assistant')
    expect(trimmed.some((message) => message.role === 'user' && message.content === '최신 질문')).toBe(true)
  })

  it('메시지 7개에서 마지막이 assistant여도 마지막 user를 교체한다', () => {
    const messages = [
      { role: 'user', content: 'u1' },
      { role: 'assistant', content: 'a1' },
      { role: 'user', content: 'u2' },
